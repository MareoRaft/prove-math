...
define(["jquery", "underscore", "browser-detect", "check-types", "katex", "mathjax", "profile", "marked", "graph", "node", "graph-animation", "blinds", "chosen", "user"], function ($, _, browser, is, katex, mathjax, undefined, marked, graph, Node, graphAnimation, blinds, chosen, user) {
	...
	graphAnimation.init({
		...
		circle_class_conditions: {
			"bright-circle": function (node) {
				return node.learned;
			},
			"axiom-circle": function (node) {
				return node.type === "axiom" || node.type === null;
			},
			"definition-circle": function (node) {
				return node.type === "definition";
			},
			"theorem-circle": function (node) {
				return node.type === "theorem";
			},
			"exercise-circle": function (node) {
	...
	blinds.init({
		...
		blind_class_conditions: {
			...
			empty: function (node, display_key, key) {
	...
	ws.onmessage = function (event) {
		...
		if (ball.command === "populate-oauth-urls") {
			...
			hide("#login");
			show("#overlay");
		} else if (ball.command === "prompt-starting-nodes") {} else if (ball.command === "load-graph") {
	...
	$("#account-type, #username, #password").keyup(function () {
		// keyup to INCLUDE whatever was just typed in .val()
		if ($(this).val() !== "") {
	...
	$(document).on("node-click", function (Event) {
		...
		updateNodeTemplateLearnedState();
		setTimeout(function () {
			// see http://stackoverflow.com/questions/35138875/d3-dragging-event-does-not-terminate-in-firefox
			blinds.open({
				object: current_node });
			hide("svg");
			hide("#overlay");
	...
	$(document).keyup(function (event) {
		if (event.which === 27 /* Esc */) {
			// right now this runs even if the blinds are NOT the frontmost thing, which could lead to unpredictable behavior
			fromBlindsToGraphAnimation();
		}
	});
	...
	function keyToDisplayKey(word, node) {
		if (word === "description") {
			return node.type;
		}if (word === "dependencies" || word === "synonyms" || word === "plurals") {
			return word;
		} // we want these to stay plural
