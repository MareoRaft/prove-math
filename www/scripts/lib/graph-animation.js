...
define(["jquery", "check-types", "profile", "d3", "user"], function ($, is, undefined, d3, user) {
	...
	function init(input) {
		...
		.links(gA.links).size([gA.width(), gA.height()]).charge(-400) // all of these can be FUNCTIONS, which act on each node or link, depending on the property :)
		// .linkDistance(120) // this is too "fixed". better to use other variables to make the spacing self-create
		.linkStrength(0.2).gravity(0.05).alpha(0.2)
		...
		gA.x = d3.scale.linear().range([0, gA.width()]);
	...
	function update() {
		// this function gets called AGAIN when new nodes come in
		...
		gA.svg.selectAll(".node-text").text(gA.node_label);
		gA.svg.selectAll("circle").classed(gA.circle_class_conditions);
	}
	...
	function mouseup(node) {
		if (mod(getShortTime(new Date()) - node.time_before, 60) < 0.85 && cartesianDistance([node.client_x_before, node.client_y_before], [d3.event.clientX, d3.event.clientY]) < 55) {
	...
	function addNodesAndLinks(_ref) {
		...
		_.each(nodes, function (node) {
			...
			} else if (_.contains(gA_node_ids, node.id)) {} else {
				new_nodes.push(node);
			}
		});
		...
		_.each(links, function (link) {
			...
			} else if (_.contains(gA_link_ids, generateLinkId(link))) {} else {
				new_links.push(link);
			}
		});
