define(["jquery", "underscore", "profile", "check-types", "graph"], function ($, _, undefined, is, graph) {
	...
	function _displayBlind(blind, $before) {
		if (def($before)) $before.before(blind.htmlified);else blinds.$window.append(blind.htmlified); // we could grab the jQuery $sel here by using last() (or possibly the return value of append()).  Then we would not need '#'+blind.id to tie the trigger.  But we *may* need blind.id for something else.  That is, resuing blind objects if we wanted to do that for some reason.
	}
