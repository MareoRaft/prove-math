...
define(["jquery", "underscore", "d3", "check-types", "profile"], function ($, _, d3, check, undefined) {
	...
	function removeNodeById(id) {
		// assumes only one node of each id exists
	...
	function removeLinksFromArrayById(link_array, id) {
		...
		for (var i = 0; i < link_array.length; i++) {
			if (link_array[i].source.id === id || link_array[i].target.id === id) {
	...
	var force = d3.layout.force().nodes(nodes) // and when we push new nodes to nodes, things happen (i think)
	.links(links).size([width, height]).charge(-200).linkDistance(40).on("tick", updateSVGNodeAndLinkPositions);
	...
	function updateSVGNodeAndLinkExistence() {
		...
		var node = svg.selectAll(".node").data(force.nodes(), function (d) {
			return d.id;
		});
		node.enter().append("circle").classed("node", true).attr({ // we may consider adding the position too. it will get updated on the next tick anyway, so we will only add it here if things look glitchy
