...
define(["jquery", "underscore", "profile", "check-types"], function ($, _, undefined, is) {
	...
	var user = {
		...
		learned_node_ids: [] };
	function init(dict) {
		// this must handle empty input before user logs in, and the real user account init when they log in
		$.extend(true, user, dict);
	}
	...
	function update_identifier(identifier) {
		if (identifier) {
			if (identifier.type !== "local") {} else if (is.not["null"](user.account.id) && user.account.id !== identifier.id) {
...
}); // the first parameter means DEEP extend
// bla bla oauth.
